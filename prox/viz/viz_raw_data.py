...
def main(args):
    # recording_dir: /qualitive/recordings/SceneName_SubjectID_SequenceID
    ...
    # 3.add <static scene mesh> & <human body point cloud> to o3d visualizer
    trans = np.eye(4)
    if args.show_scene:
    ...
    for img_name in sorted(os.listdir(color_dir))[args.start::args.step]:
        ...
        # (2).human body point cloud
        if args.show_scan:
            # a.depth image
            depth_img = cv2.imread(os.path.join(depth_dir, img_name + '.png'), -1)\
                .astype(float)
            depth_img /= 8.0
            depth_img /= 1000.0
            ...
            if args.show_body_only:
                if args.mask_on_color:
                    mask = cv2.imread(os.path.join(bodyIndexColor_dir, img_name + '.png'),
                                      cv2.IMREAD_GRAYSCALE)
                else:
                    mask = cv2.imread(os.path.join(bodyIndex_dir, img_name + '.png'),
                                      cv2.IMREAD_GRAYSCALE)
            ...
            # c.human body point cloud
            if args.show_color:
                scan_dict = projection.create_scan(mask, depth_img,
                                                   mask_on_color=args.mask_on_color,
                                                   color_im=color_img[:, :, ::-1])
            else:
                scan_dict = projection.create_scan(mask, depth_img,
                                                   mask_on_color=args.mask_on_color)
            ...
            # human body point cloud: camera -> world
            scan.transform(trans)

        # (3).update o3d visualizer
        vis.update_geometry()
...
if __name__ == '__main__':
    ...
    parser.add_argument('recording_dir',
                        ...
                        default=os.getcwd(),
                        help='recording dir')
    parser.add_argument('--show_scene',
                        default=True,
                        type=lambda arg: arg.lower() in ['true', '1'],
                        help='')
    parser.add_argument('--show_scan',
                        default=True,
                        type=lambda arg: arg.lower() in ['true', '1'],
                        help='')
    parser.add_argument('--show_body_only',
                        default=False,
                        type=lambda arg: arg.lower() in ['true', '1'],
                        help='')
    parser.add_argument('--mask_on_color',
                        ...
                        type=lambda arg: arg.lower() in ['true', '1'],
                        help='')
    parser.add_argument('--show_color',
                        default=False,
                        type=lambda arg: arg.lower() in ['true', '1'],
                        help='')
